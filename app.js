...
// ✅ Session middleware (now with Mongo store)
app.use(session({
...
app.post('/api/login', async (req, res) => {
    ...
    if (email && password) {
        ...
        res.status(400).send({ result: 'Enter valid email and password' });
    }
});
...
app.post('/api/owner_login', async (req, res) => {
    ...
    if (email && password) {
        ...
        res.status(400).send({ result: 'Enter valid email or password' });
    }
});
...
app.get('/api/profile', async (req, res) => {
    try {
        ...
        const { email, type } = req.session.user;
        let user;
        if (type === 'user') {
